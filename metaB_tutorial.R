...
### 9th Metabarcoding school - Colombia 
### AI/ML analysis of metabarcoding data for biomonitoring - 1st-6th November 2019 
# Tristan Cordier

########################################### Setting the stage #############################################

# setting the working directory
setwd("~/Desktop/Metabarcoding school/")
...
source("custom_functions/sml_compo_hyperparameter_fitting.R")
...
# taxonomic barplot
# By locality
bar_plot(OTUr, MET, aggreg = c("Locality"), taxo_file = taxo, title_ = "By locality",
         file_name = "Taxo_barplot1", font_size_ = 0.7, tax_rank = 6, pdf = T)

# high taxonomic rank
bar_plot(OTUr, MET, aggreg = c("Station"), taxo_file = taxo, title_ = "By station",
         file_name = "Taxo_barplot2", font_size_ = 0.7, metazoan = T,
         tax_rank = 10, pdf = T)

## check one group in particular
nematoda <- grep("Nematoda", taxo[,"taxon"])
OTU_nematoda <- OTUr[,nematoda]
taxo_nematoda <- taxo[nematoda,]

bar_plot(OTU_nematoda, MET, aggreg = c("Station"), taxo_file = taxo_nematoda, title_ = "Nematoda by station",
         file_name = "Taxo_barplot3", font_size_ = 0.7, tax_rank = 10, lastrank = T, pdf = T, reads_percentage = T)
...
res <- plot_ml(preds, MET, index = "AMBI", title = "RForest_predictions.pdf", 
               aggreg = c("Grab", "Station", "Locality"), pdf = T)

# we can do that within a loop and export the plots
for (BI in c("AMBI", "NSI", "ISI", "NQI1", "Shannon"))
{
  ## SML on the 10 most abundant only for doing it fast...
  ...
  plot_ml(preds, MET, index = BI, aggreg = c("Grab", "Station", "Locality"), pdf = T,
          title = paste("RF_10_Predictions_", BI, ".pdf", sep=""))
}
...
for (i in feat)
{
  ...
  stats <- plot_ml(data = preds$predictions, metadata = MET_te, index = "AMBI",  title = paste("RF_", i, "features", sep=""),
                   aggreg = c("Grab", "Station", "Locality"), folder_export = "features_extraction", pdf = T)
...
### Let's do with hyperparameter tuning
## how many combinations of parameter to try?
hyperparam <- 20

## selecting the 100 best OTUs
## let's train a model will all the features for the AMBI index
mod_all_feature <- ranger(MET[,"AMBI"] ~ ., data=OTU, mtry=floor(dim(OTU)[2]/3),
                          num.trees = 300, importance= "impurity", write.forest = T)
## sort the OTUs by decreasing importance
imp <- sort(mod_all_feature$variable.importance, decreasing = T)
OTU_reduced <- OTU[,names(imp[1:50])]
...
# RF
preds_hyp_rf <- sml_compo_hyper_fit(OTU_reduced, MET, index = "AMBI", algo = "RF", grid = grid_rf)
plot_ml(preds_hyp_rf$preds, metadata = MET, index = "AMBI", aggreg = c("Grab", "Station", "Locality"), folder_export = paste0("RF_100hypOpt"), pdf = T)
write.table(preds_hyp_rf$RMSE, paste0("RF_100hypOpt/RMSE.tsv"), sep="\t", row.names = F, quote = F, dec=",")
# SVM
preds_hyp_svm <- sml_compo_hyper_fit(OTU_reduced, MET, index = "AMBI", algo = "SVM", grid = grid_svm)
plot_ml(preds_hyp_svm$preds, metadata = MET, index = "AMBI", aggreg = c("Grab", "Station", "Locality"), folder_export = paste0("SVM_100hypOpt"), pdf = T)
write.table(preds_hyp_svm$RMSE, paste0("SVM_100hypOpt/RMSE.tsv"), sep="\t", row.names = F, quote = F, dec=",")


# and compare without optmization
preds <- sml_compo(OTU_reduced, MET, index = "AMBI", algo = "SVM", cross_val = "Locality")

# plot the independant predictions against the real values and collect stats
res <- plot_ml(preds, MET, index = "AMBI", title = "SVM_noOpt", folder_export = "SVM_100hypOpt",
               aggreg = c("Grab", "Station", "Locality"), pdf = T)
