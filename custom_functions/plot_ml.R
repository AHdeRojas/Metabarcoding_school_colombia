...
plot_ml <- function(data, metadata, index, title = NULL, aggreg = NULL, pdf = F, folder_export = NULL, dots_size = 1, font_size = 1) {
  ...
  combi1 <- data
  combi2 <- comp[,index]
  # folder_export is used to export plots for a list of taxonomic groups
  if (is.null(folder_export)) folder_export <- "MLplots"
  ...
  if (!is.null(aggreg))
  {
    ...
    if (length(aggreg)==1) 
    {
      ...
      # farm name
      agg_nam <-  aggregate(combi1, by=list(comp[,aggreg[1]]), FUN=mean)[,"Group.1"]
      # col plot
    ...
    if (length(aggreg)==2) 
    {
      ...
      # agg name
      agg_nam <-  aggregate(combi1, by=list(comp[,aggreg[1]], comp[,aggreg[2]]), FUN=mean)[,"Group.2"]
      # col plot
    ...
    if (length(aggreg)==3) 
    {
      ...
      # farm name
      agg_nam <-  aggregate(combi1, by=list(comp[,aggreg[1]], comp[,aggreg[2]], comp[,aggreg[3]]), FUN=mean)[,"Group.3"]
      # col plot
  ...
  ## normalized Total Organic Matter
  if (index == "N.TOC") 
  {
    for (i in 1:length(combined1))
    {
      if (combined1[i] < 20) comb1_disc[i] <- 1
      else if (combined1[i] >= 20 && combined1[i] < 27) comb1_disc[i] <- 2
      else if (combined1[i] >= 27 && combined1[i] < 34) comb1_disc[i] <- 3
      else if (combined1[i] >= 34 && combined1[i] < 41) comb1_disc[i] <- 4
      else if (combined1[i] >= 41 && combined1[i] <= 200) comb1_disc[i] <- 5
      else if (combined1[i] > 200) comb1_disc[i] <- 5 
      
      if (combined2[i] < 20) comb2_disc[i] <- 1
      else if (combined2[i] >= 20 && combined2[i] < 27) comb2_disc[i] <- 2
      else if (combined2[i] >= 27 && combined2[i] < 34) comb2_disc[i] <- 3
      else if (combined2[i] >= 34 && combined2[i] < 41) comb2_disc[i] <- 4
      else if (combined2[i] >= 41 && combined2[i] <= 200) comb2_disc[i] <- 5
      else if (combined2[i] > 200) comb2_disc[i] <- 5 
    }
    ## regression
    diff <- comb1_disc-comb2_disc 
    good_ass <- length(subset(diff, diff ==0))
  ...
  if (index == "AMBI") 
  {
    if(pdf)
    {
      dir.create(file.path(getwd(), folder_export), showWarnings = F)
      pdf(file = paste(folder_export, "/",  title, "_AMBI.pdf", sep=""), width = 6.6, height=4, useDingbats=F)
    ...
    plot(combined1 ~ combined2, xlim=c(0,6), ylim=c(0,6), pch =col, col= col,  xaxs="i",yaxs="i",cex= dots_size, main = paste("AMBI prediction /", title), xlab = "Morphology", ylab = "Molecular")
    ...
    legend("topleft", as.vector(unique(agg_nam)), col= unique(col), pch=unique(col), box.lty=0, inset=c(0,0.11), cex= font_size)
  ...
  if (index == "microAMBI") 
  {
    ...
    {
      dir.create(file.path(getwd(), folder_export), showWarnings = F)
      pdf(file = paste(folder_export, "/",  title, "_microAMBI.pdf", sep=""), width = 6.6, height=4, useDingbats=F)
    } else quartz(width = 6.6, height=4)
    #par(mfrow = c(1, 2))
    mat <- rbind(c(1,2,3), c(1,2,4))
    layout(mat, widths=c(2.5,1,1))
    ...
    #col2 <- col[as.integer(as.factor(farm_nam))]
    #col <-  comp_foram$Col_plot
    plot(combined1 ~ combined2, xlim=c(0,6), ylim=c(0,6), pch =col, col= col,  xaxs="i",yaxs="i",cex=dots_size, main = paste("microAMBI prediction /", title), xlab = "Morphology", ylab = "Molecular")
  ...
  if (index == "NSI") 
  {
    ...
    {
      dir.create(file.path(getwd(), folder_export), showWarnings = F)
      pdf(file = paste(folder_export, "/",  title, "_NSI.pdf", sep=""), width = 6.6, height=4, useDingbats=F)
    } else quartz(width = 6.6, height=4)
    #par(mfrow = c(1, 2))
    mat <- rbind(c(1,2,3), c(1,2,4))
    layout(mat, widths=c(2.5,1,1))
    ...
    plot(combined1 ~ combined2, xlim=c(0,31), ylim=c(0,31), pch =col, col= col,  xaxs="i",yaxs="i",cex=dots_size, main = paste("NSI prediction /", title), xlab = "Morphology", ylab = "Molecular")
    arrows(combined2, combined1-combined1_sd, combined2, combined1+combined1_sd, length=0.01, angle=90, code=3, col=col)
    abline(mod, col="blue")
  ...
  if (index == "NQI1") 
  {
    ...
    {
      dir.create(file.path(getwd(), folder_export), showWarnings = F)
      pdf(file = paste(folder_export, "/",  title, "_NQI1.pdf", sep=""), width = 6.6, height=4, useDingbats=F)
    } else quartz(width = 6.6, height=4)
    #par(mfrow = c(1, 2))
    mat <- rbind(c(1,2,3), c(1,2,4))
    layout(mat, widths=c(2.5,1,1))
    ...
    plot(combined1 ~ combined2, xlim=c(0,1), ylim=c(0,1), pch =col, col= col,  xaxs="i",yaxs="i",cex=dots_size, main = paste("NQI1 prediction /", title), xlab = "Morphology", ylab = "Molecular")
    arrows(combined2, combined1-combined1_sd, combined2, combined1+combined1_sd, length=0.01, angle=90, code=3, col=col)
    abline(mod, col="blue")
  ...
  if (index == "ISI") 
  {
    ...
    {
      dir.create(file.path(getwd(), folder_export), showWarnings = F)
      pdf(file = paste(folder_export, "/",  title, "_ISI.pdf", sep=""), width = 6.6, height=4, useDingbats=F)
    } else quartz(width = 6.6, height=4)
    #par(mfrow = c(1, 2))
    mat <- rbind(c(1,2,3), c(1,2,4))
    layout(mat, widths=c(2.5,1,1))
    ...
    plot(combined1 ~ combined2, xlim=c(0,13), ylim=c(0,13), pch =col, col= col,  xaxs="i",yaxs="i",cex=dots_size, main = paste("ISI prediction /", title), xlab = "Morphology", ylab = "Molecular")
    arrows(combined2, combined1-combined1_sd, combined2, combined1+combined1_sd, length=0.01, angle=90, code=3, col=col)
    abline(mod, col="blue")
  ...
  if (index == "Shannon") 
  {
    ...
    {
      dir.create(file.path(getwd(), folder_export), showWarnings = F)
      pdf(file = paste(folder_export, "/", title, "_Shannon.pdf", sep=""), width = 6.6, height=4, useDingbats=F)
    } else quartz(width = 6.6, height=4)
    #par(mfrow = c(1, 2))
    mat <- rbind(c(1,2,3), c(1,2,4))
    layout(mat, widths=c(2.5,1,1))
    ...
    plot(combined1 ~ combined2, xlim=c(0,5.7), ylim=c(0,5.7), pch =col, col= col,  xaxs="i",yaxs="i",cex=dots_size, main = paste("Shannon prediction /", title), xlab = "Morphology", ylab = "Molecular")
    arrows(combined2, combined1-combined1_sd, combined2, combined1+combined1_sd, length=0.01, angle=90, code=3, col=col)
    abline(mod, col="blue")
  ...
  if (index == "N.TOC") 
  {
    ...
    {
      dir.create(file.path(getwd(), folder_export), showWarnings = F)
      pdf(file = paste(folder_export, "/", title, "_N.TOC.pdf", sep=""), width = 6.6, height=4, useDingbats=F)
    } else quartz(width = 6.6, height=4)
    #par(mfrow = c(1, 2))
    mat <- rbind(c(1,2,3), c(1,2,4))
    layout(mat, widths=c(2.5,1,1))
    ...
    plot(combined1 ~ combined2, xlim=c(0,200), ylim=c(0,200), pch =col, col= col,  xaxs="i",yaxs="i", cex=dots_size, main = paste("N.TOC prediction /", title), xlab = "Morphology", ylab = "Molecular")
    arrows(combined2, combined1-combined1_sd, combined2, combined1+combined1_sd, length=0.01, angle=90, code=3, col=col)
    abline(mod, col="blue")
    rect(0,0,20,20,border="blue")
    rect(20,20,27,27,border="green")
    rect(27,27,34,34,border="yellow")
    rect(34,34,41,41,border="orange")
    rect(41,41,200,200,border="red")
    
    text(200,20, pos=2,paste("R²=", round(summary(mod)$adj.r.squared, 3), sig, sep=""))
    text(200,10, pos=2,paste("Kappa=", round(kap$value, 3), sigk, sep=""))
  ...
  #print(as.character(farm_nam))
  
  return(list("preds_cont" = combined1,
              "bi_values"= combined2,
              "preds_cat" = comb1_disc,
              "labels" = comb2_disc,
              "R2" = paste(round(summary(mod)$adj.r.squared, 3), sig, sep=""),
              "KAP" = paste(round(kap$value, 3), sigk, sep=""),
              "farms" = agg_nam))
