...
bar_plot <- function(otu_table, comp_, aggreg = F, taxo_file, title_, font_size_ = 0.8, horizontal_ = T,
                     ...
                     remove_rare = T, remove_unass = F, distinct_colors = F, functional = F, reads_percentage = F) {
  ...
  # pdf <- T
  # taxo_group <- taxo_group
  # aggreg <- c("basin", "molecule", "dataset")
  # remove_rare = T
  # remove_unass = F
  # functional = F
  # distinct_colors <- F
  ...
  # 
  # collect stats number of OTUs and total sum
  otuNum <- ncol(otu)
  otuSum <- sum(otu)

  # need to remove [ ] or weird character in taxo file if any
  #for (i in 1:length(tax)) tax[i,"taxon"] <- gsub("[^[:alnum:][:blank:]_;+?&/\\-]", "", tax[i,"taxon"], c) 
  
  # if the column "taxon" is a factor, convert it into character
  if (is.factor(tax[,"taxon"])) tax[,"taxon"] <- as.character(tax[,"taxon"])
  ...
  # collect stats per aggreg combi
  combSumRelAb <- round((rowSums(dat) / otuSum) *100, 3)
  if (reads_percentage) rownames(dat) <- paste0(rownames(dat), "\n", combSumRelAb, "%")
